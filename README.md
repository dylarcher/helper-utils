...
This section outlines how to contribute to the development of
`@dylarcher/js-helpers`, including understanding the project structure and using
...
The `npm run build` script handles the compilation (using `tsc` - the TypeScript
compiler, even for JS projects to leverage its build capabilities and type
definition generation) and generates the necessary output files in `dist/`,
including type definitions (`types.d.ts`). The `prepublishOnly` script
automatically runs these before publishing.
