...
describe('encrypt(text, key, iv) - Error Handling and Input Validation', () => {
	...
	const validKey = crypto.randomBytes(32); // AES-256 needs 32-byte key
	const validIv = crypto.randomBytes(16);  // AES-CBC needs 16-byte IV

	it('should throw an error for invalid key length', () => {
		const shortKey = crypto.randomBytes(16); // Invalid length
		const longKey = crypto.randomBytes(48);  // Invalid length

		assert.throws(
			() => encrypt(testText, shortKey, validIv),
			(err) => err.message.includes('Invalid key length') || err.code === 'ERR_CRYPTO_INVALID_KEYLEN',
		...
		assert.throws(
			...
			(err) => err.message.includes('Invalid key length') || err.code === 'ERR_CRYPTO_INVALID_KEYLEN' || err.code === 'ERR_INVALID_ARG_TYPE',
			'Should throw for a key of incorrect type/length'
		);
	});

	it('should throw an error for invalid IV length', () => {
		const shortIv = crypto.randomBytes(8); // Invalid length
		const longIv = crypto.randomBytes(24);  // Invalid length

		assert.throws(
			() => encrypt(testText, validKey, shortIv),
			(err) => err.message.includes('Invalid IV length') || err.code === 'ERR_CRYPTO_INVALID_IVLEN',
		...
		assert.throws(
			...
			(err) => err.message.includes('Invalid IV length') || err.code === 'ERR_CRYPTO_INVALID_IVLEN' || err.code === 'ERR_INVALID_ARG_TYPE',
    ...
    it('should throw if key is not a Buffer', () => {
        assert.throws(
            ...
            (err) => err.code === 'ERR_INVALID_ARG_TYPE' || err.message.includes('Key must be a buffer'),
    ...
    it('should throw if IV is not a Buffer', () => {
        assert.throws(
            ...
            (err) => err.code === 'ERR_INVALID_ARG_TYPE' || err.message.includes('IV must be a buffer'),
